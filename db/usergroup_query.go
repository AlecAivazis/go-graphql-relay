...
type UserGroupQuery struct {
	...
	loadTotal  []func(context.Context, []*UserGroup) error
...
func (ugq *UserGroupQuery) QueryAdmin() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, usergroup.AdminTable, usergroup.AdminColumn),
		)
...
func (ugq *UserGroupQuery) WithAdmin(opts ...func(*UserQuery)) *UserGroupQuery {
	query := (&UserClient{config: ugq.config}).Query()
...
func (ugq *UserGroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserGroup, error) {
	...
	if query := ugq.withAdmin; query != nil {
		if err := ugq.loadAdmin(ctx, query, nodes, nil,
			func(n *UserGroup, e *User) { n.Edges.Admin = e }); err != nil {
			return nil, err
		}
	}
	for i := range ugq.loadTotal {
		if err := ugq.loadTotal[i](ctx, nodes); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ugq *UserGroupQuery) loadAdmin(ctx context.Context, query *UserQuery, nodes []*UserGroup, init func(*UserGroup), assign func(*UserGroup, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserGroup)
	for i := range nodes {
		if nodes[i].user_group_admin == nil {
			continue
		}
		fk := *nodes[i].user_group_admin
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_group_admin" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
